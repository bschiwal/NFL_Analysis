...
passtrailingend <-pbp%>% filter(qb_dropback==1,
                                ...
                                half_seconds_remaining<=240 | game_half=="Overtime")
...
qbclutch<-ggplot(passEPAtrailend, aes(cpoe, qb_epa_per_play))+
  ...
  labs(x= "Completion % Over Expectation (CPOE)", 
       ...
       title="Quarterback Clutch Efficency 2024", 
       subtitle ="Dropback Passes in Final Four Minutes of Half or Overtime When Trailing by < 8 Points; Min 20 Attempt", 
